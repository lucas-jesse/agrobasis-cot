...
# Configuração padrão dos gráficos: mantém a barra de ferramentas ativa
# para permitir download em PNG direto pelo usuário.
PLOTLY_CONFIG = {
    "displayModeBar": True,
    "displaylogo": False,
    "responsive": True,
    "toImageButtonOptions": {
        "format": "png",
        "filename": "agrobasis_cot_grafico",
        "scale": 5
    },
...
def load_price_data(commodity: str, start_date, end_date) -> pd.DataFrame:
    ...
    try:
        ...
        out["data_preco"] = pd.to_datetime(out["data_preco"]).dt.tz_localize(None).astype("datetime64[ns]")
...
if buscar_preco:
    ...
    if not price_df.empty:
        ...
        price_sorted = price_df.sort_values("data_preco").copy()
        # Normaliza o dtype das colunas de data para evitar MergeError em versões
        # recentes do pandas, que passaram a exigir resoluções de datetime idênticas.
        df_sorted["data"] = df_sorted["data"].astype("datetime64[ns]")
        price_sorted["data_preco"] = price_sorted["data_preco"].astype("datetime64[ns]")
...
# Controle global de tamanho dos gráficos.
# Útil para apresentação, projeção em tela grande e download em PNG com melhor leitura.
tamanho_grafico = st.radio(
    "Tamanho dos gráficos",
    ["Normal", "Grande", "Apresentação"],
    horizontal=True,
    index=0,
    help="Aumenta a altura dos gráficos no dashboard. O botão de câmera no canto do gráfico baixa a imagem em PNG."
)
_fator_altura = {"Normal": 1.0, "Grande": 1.25, "Apresentação": 1.55}[tamanho_grafico]

def altura(base):
    return int(base * _fator_altura)
...
with tab1:
    ...
    with col_a:
        ...
        fig1.update_layout(**CHART_LAYOUT, barmode="overlay", yaxis_title="Contratos", hovermode="x unified", height=altura(390))
    ...
    with col_b:
        ...
        figp.update_layout(**CHART_LAYOUT, hovermode="x unified", height=altura(390))
        figp.update_layout(
            yaxis=dict(
                title="Net · Contratos",
                tickformat=".2s",
    ...
    with col_c:
        ...
        fig3.update_layout(**CHART_LAYOUT, yaxis_title="Contratos", hovermode="x unified", height=altura(380))
    ...
    with col_d:
        ...
        fig2.update_layout(**CHART_LAYOUT, yaxis_title="Contratos", hovermode="x unified", height=altura(380))
    ...
    with col_e:
        ...
        fig4.update_layout(**CHART_LAYOUT, xaxis_title="Semana do ano", yaxis_title="Contratos", hovermode="x unified", height=altura(370))
    ...
    with col_f:
        ...
        fig_oi.update_layout(**CHART_LAYOUT, hovermode="x unified", height=altura(370))
        ...
        st.plotly_chart(fig_oi, use_container_width=True, config=PLOTLY_CONFIG)
...
with tab2:
    ...
    fig_heat.update_layout(**CHART_LAYOUT, height=altura(520), xaxis_title="Mês", yaxis_title="Ano")
    ...
    fig_m.update_layout(**CHART_LAYOUT, height=altura(340), yaxis_title="Contratos")
...
with tab3:
    ...
    styled = df_table.style.map(highlight_net, subset=[c for c in ["Net", "Var. Semanal Net"] if c in df_table.columns]).format(na_rep="—", thousands=".", decimal=",")
    st.dataframe(styled, use_container_width=True, height=altura(520))
